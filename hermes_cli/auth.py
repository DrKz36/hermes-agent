...
# The explicit no-tools CLI relies only on the built-in registry.  Importing
# this extension registry would scan and execute provider plugins before a
# constrained chat has a chance to prove its empty tool surface.
try:
...
def resolve_provider(
    ...
    *,
    ...
    # hardcoded dict above remains authoritative for existing ones. The
    # no-tools policy intentionally keeps that extension registry untouched.
    if not _no_tools_active():
        try:
